...
pub fn apply(
    ...
    // `schemes` repo plus any `[[schemes.extras]]` (an extra overrides the
    // built-in on a duplicate); custom schemes are the locally generated ones. A
    // name present in both a scheme repo and the custom directory is ambiguous
    // and rejected.
    let scheme_system =
