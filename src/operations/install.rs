...
fn install_git_url(data_item_path: &Path, item_name: &str, item_git_url: &str) -> Result<()> {
...
fn install_dir(data_item_path: &Path, item_name: &str, item_path: &Path) -> Result<()> {
...
/// any other command
pub fn install(config_path: &Path, data_path: &Path) -> Result<()> {
    // Hooks
    ...
    for item in items {
        ...
        match Url::parse(item.path.as_str()) {
            Ok(_) => install_git_url(&data_item_path, item.name.as_str(), item.path.as_str())?,
            Err(_) => install_dir(&data_item_path, item.name.as_str(), &item_path)?,
        }
    }

    // Schemes
    let schemes_repo_path = hooks_path.join(SCHEMES_REPO_NAME);
    install_git_url(&schemes_repo_path, SCHEMES_REPO_NAME, SCHEMES_REPO_URL)?;
