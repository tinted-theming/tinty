...
const FONT_SPACE_MONO_700: &[u8] = include_bytes!("studio/fonts/space-mono-700.woff2");

// The studio reuses the gallery's code snippets so previews render with the
// exact same highlighting tokens the gallery uses.
...
pub fn studio(data_path: &Path, dump_dir: Option<&str>, should_open: bool) -> Result<PathBuf> {
    let output_dir = dump_dir.map_or_else(
        || data_path.join(ARTIFACTS_DIR).join(STUDIO_DIR_NAME),
        PathBuf::from,
    );

    // The studio works fully without a scheme library; when one is available we
    // embed it so the "Start from" picker and deep links can offer known schemes.
    let schemes_json = schemes_dir_path(data_path, false)
        .and_then(|path| scheme_entries_json(&path))
        .unwrap_or_else(|_| "[]".to_string());

    write_studio_files(&output_dir, &schemes_json)?;
    ...
    println!("Studio written to {}", index_path.display());

    Ok(index_path)
}

fn write_studio_files(output_dir: &Path, schemes_json: &str) -> Result<()> {
    ...
    write_binary_file(fonts_dir.join("space-mono-700.woff2"), FONT_SPACE_MONO_700)?;
...
fn open_in_browser(index_path: &Path) -> Result<()> {
    ...
    let status = command
        ...
        .stderr(Stdio::null())
        .status()
        .with_context(|| format!("Unable to open studio at {}", index_path.display()))?;

    if !status.success() {
        return Err(anyhow::anyhow!(
            "Unable to open studio at {}",
            index_path.display()
        ));
    }
